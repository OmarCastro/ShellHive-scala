...
import shared.api.{WSRequest, WSResponse}

import scala.util.Try

/**
 * WebSocketsAutowireClient
 *
 * autowire client that uses web sockets
 *
 * Note: When using this, don't forget to import autowire._ even though it doesn't seem like it's needed
 */
trait WebSocketsAutowireClient extends autowire.Client[String, upickle.default.Reader, upickle.default.Writer]{
  ...
  override def doCall(req: Request): Future[String] = {
    ...
    socket.send(upickle.default.write(new WSRequest[Request](WebSocketsAutowireClient.id,req)))
    newPromise.future
  }
